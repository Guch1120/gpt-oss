...
options: 推論パラメータ。これらを調整することで推論の質を制御。
    - temperature: 応答のランダム性を制御。0に近いほどより決定的で一貫性のある応答を生成。
        論理的な推論や事実に基づいた回答を求める場合は、0.2〜0.7程度の低い値が推奨らしい。
    - num_ctx: モデルが一度に考慮できるコンテキストのトークン数。デフォルトは2048、大きくする（例: 4096）ことでより長いプロンプトや複雑な文脈を理解しやすくなるらしい。
    - repeat_penalty: 同じ単語やフレーズが繰り返し出現するのを防ぐ。1.0がデフォルトで少し大きい値（例: 1.1）にすると繰り返しが抑制されます。
より良い推論結果を得るためには、特に `temperature` を低めに設定し、`num_ctx` を増やすことが効果的らしい。
"""
